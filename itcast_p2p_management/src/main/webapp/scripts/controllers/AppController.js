...
angular
				...
				function($rootScope, $scope, $state, AdminService) { // 将AdminService注入到了当前的loginCtrl中。
					...
					$scope.login = function(username, password) {
						var str = 'username=' + username + '&password='
								+ password; // username=tom&password=123

						// $http.post('/itcast_p2p_action/account/login’,username=tom&password=123,option.header).success(function(response);
		...
		.controller(
				...
				function($scope, $state, $stateParams, ProductService) {
					var proId = $stateParams.proId; // 真正的接收路由传递的参数
					var str = "proId=" + proId
					...
					$scope.commitProduct = function(proId) {
						...
						for ( var x in $scope.productInfo) {
							if (x === "proEarningRate") {
								...
								str += "&" + x + "=" + $scope.productInfo[x]; // &proNum=xxx&proName=xxx
							}
						}
						if (str.substring(0, 1) === '&') {
							str = str.substring(1);
						}
						alert(str); // &proNum=xxx&proName=xxx&proEarningRates={6:2.5,12:3.0,18:3.5}
		...
		.controller(
				...
				function($scope, $state, AuthService, PostService, hmd) {
					...
					init_query();
					// $scope.queryData = getDataByParams;
					// 开始匹配
